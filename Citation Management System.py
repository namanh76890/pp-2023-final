...
class MyHTMLParser:
    ...
    def get_author_and_publication_year(self, url):
        try:
            ...
            if doi and doi['content']:
                self.doi = doi['content']
            if publisher and publisher['content']:
                self.publisher = publisher['content']
...
class Citation:
    ...
    def authors_to_string(self):
        return '; '.join(', '.join(author) for author in self.authors)


    def string_to_authors(self, authors_str):
...
class CitationManager:
    ...
    def add_citation(self, citation):
        ...
        """, (citation.authors_to_string(), citation.title, citation.publication_year, citation.publisher, citation.doi, citation.url))

        conn.commit()
        conn.close()

    def get_citations_from_db(self):
        conn = sqlite3.connect("citations.db")
        cursor = conn.cursor()
    ...
    def remove_citation(self, citation_id):
        conn = sqlite3.connect("citations.db")
        cursor = conn.cursor()
    ...
    def search_citations(self, query):
        ...
        cursor.execute("SELECT * FROM citations WHERE title LIKE ? OR authors LIKE ?", (f'%{query}%', f'%{query}%'))
    ...
    def generate_bibliography(self, style):
        ...
        cursor.execute("SELECT authors, title, publisher, publication_year, url, doi FROM citations")
        ...
        for citation_tuple in citations:
            citation = Citation(*citation_tuple)
...
def format_citation(citation, style):
    ...
    citation.authors = citation.authors.split(', ')
...
class CitationManagerApp(tk.Tk):
    ...
    def update_citation_listbox(self):
        self.citation_listbox.delete(0, tk.END)
        citations = self.manager.get_citations_from_db()
        for citation in citations:
            self.citation_listbox.insert(tk.END, f"{citation[2]} | {citation[1]}, {citation[3]}")
            self.manager.citations.append(citation)
    ...
    def remove_citation(self):
        ...
        self.manager.remove_citation(selected_citation[0])
        self.update_citation_listbox()
    ...
    def search_citations(self):
        ...
        for result in results:
            self.citation_listbox.insert(tk.END, f"{result[2]} | {result[1]}, {result[3]}")
